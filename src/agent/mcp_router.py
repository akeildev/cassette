...
class McpToolRouter:
    ...
    async def _connect_server(self, server_id: str, server_config: dict):
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Initialization failed for {server_id}: {e}")
                # Check if the server is still responsive by trying to list tools
                try:
                    await self._rpc(server_id, "tools/list", {}, 3.0)
                    logger.info(f"✓ Server {server_id} is responsive despite init failure")
                except Exception as list_error:
                    logger.error(f"Server {server_id} is not responsive: {list_error}")
                    # Disconnect the unresponsive server
            ...
            # Don't add to clients if connection failed
    ...
    async def _refresh_tools(self):
        ...
        for server_id, client in list(self.clients.items()):
            try:
                ...
                # Mark server as unhealthy and disconnect
        ...
        if self.tools:
            ...
            for tool in self.tools[:10]:  # Show first 10 tools
                logger.info(f"  - {tool.name}: {tool.description[:50]}...")
            if len(self.tools) > 10:
                logger.info(f"  ... and {len(self.tools) - 10} more tools")
    ...
    async def start(self):
        ...
        logger.info(f"Successfully connected to {successful_connections}/{len(servers)} servers")
        
        # Refresh tool list from connected servers
    ...
    async def _hot_reload_loop(self):
        ...
        while True:
            try:
                ...
                wanted_servers = set(server_config.get("id", "unknown") for server_config in self.config.get("servers", []))
